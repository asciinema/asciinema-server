defmodule Asciinema.LiveStream do
  ...
  def feed(stream_id, data) do
    :ok = GenServer.call(via_tuple(stream_id), {:feed, data})
  end
  ...
  def join(stream_id) do
    ...
    receive do
      {^ref1, pid, {_vt_size, _vt_state, _epoch} = stream_state} ->
  ...
  def init(stream_id) do
    ...
    {:ok, %{stream_id: stream_id, vt: vt, vt_size: vt_size, epoch: Timex.now()}}
  end
  ...
  def handle_call({:reset, {cols, rows} = vt_size}, _from, state) do
    ...
    {:reply, :ok, %{state | vt: vt, vt_size: vt_size, epoch: Timex.now()}}
  end

  def handle_call({:feed, {time, data}}, _from, state) do
    :ok = Vt.feed(state.vt, data)
    publish(state.stream_id, {:feed, {time, data}})
  ...
  def handle_cast({:join, {pid, ref1, ref2}}, state) do
    ...
    send(pid, {ref1, self(), {state.vt_size, Vt.dump(state.vt), state.epoch}})
